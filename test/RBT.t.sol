...
contract RedBlackTest is Test {
    ...
    function insertRBtree(RedBlackTreeLib.Tree storage rb_tree, uint256 val, bool debug) internal returns (uint256) {
    ...
    function insertBPBtree(BokkyPooBahsRedBlackTreeLibrary.Tree storage bpb_tree, uint256 val, bool debug)
        internal
        returns (uint256)
    {
        uint256 GAS_BEFORE = gasleft();
    ...
    uint256[] input_insert;
    uint256[] input_delete;
    ...
    function setUp() public {
        ...
        for (uint256 i; i < 1000; i++) {
            temp = uint256(keccak256(abi.encode(temp)));
